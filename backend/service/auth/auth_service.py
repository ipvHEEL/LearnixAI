...
class AuthService:
    def __init__(self) -> None:
        ...
        self.local_outbox_path = Path(os.getenv("LOCAL_OUTBOX_PATH", str(base_dir / "data" / "mail_outbox.log")))
    ...
    def _send_password_reset_email(self, target_email: str, reset_token: str) -> bool:
        ...
        try:
            if self.smtp_host:
                ...
                self.logger.info("Password reset email sent to %s via SMTP host %s", target_email, self.smtp_host)
                return True

            # Dev fallback: keep messages in local outbox file when SMTP is not configured.
            self.local_outbox_path.parent.mkdir(parents=True, exist_ok=True)
            with self.local_outbox_path.open("a", encoding="utf-8") as outbox:
                outbox.write(f"\n----- {datetime.now(timezone.utc).isoformat()} -----\n")
                outbox.write(message.as_string())
                outbox.write("\n")
            self.logger.warning(
                "SMTP_HOST is not configured. Password reset email for %s was written to %s",
                target_email,
                self.local_outbox_path,
            )
            return True
        except Exception:
            self.logger.exception("Failed to send password reset email to %s", target_email)
            return False
    ...
    def request_password_reset(self, email: str) -> bool:
        ...
        if not is_sent:
            ...
            self.logger.error("Password reset email delivery failed for user_id=%s", user.user_id)
        return is_sent
