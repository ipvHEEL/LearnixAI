...
class UserRepository:
    ...
    def _ensure_schema(self) -> None:
        with self._connect() as conn:
            ...
            if "avatar_image" not in columns:
                conn.execute("ALTER TABLE user_profile ADD COLUMN avatar_image TEXT")
    ...
    def get_profile(self, user_id: int) -> dict:
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT first_name, last_name, country, city, avatar_image
                FROM user_profile
                WHERE user_id = ?
                """,
                (user_id,),
        ...
        return {
            ...
            "avatar_image": row["avatar_image"] if row else "",
            "interests": interests,
        }
    ...
    def update_profile(
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO user_profile (user_id, first_name, last_name, country, city, avatar_image)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(user_id) DO UPDATE SET
                ...
                """,
                (user_id, cleaned_first_name, cleaned_last_name, cleaned_country, cleaned_city, avatar_image.strip()),
            )
