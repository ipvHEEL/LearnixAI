...
def request_password_recovery(data: PasswordRecoveryRequest):
    reset_token = auth_service.request_password_reset(str(data.email))

    if reset_token is None:
        # Prevents account/email enumeration.
        return {
            "status": "ok",
            "message": "If an account with that email exists, reset instructions have been created.",
        }

    return {
        "status": "ok",
        "message": "Password reset token created. Use it to set a new password.",
        "reset_token": reset_token,
        "expires_in_minutes": auth_service.password_reset_ttl_minutes,
    }
